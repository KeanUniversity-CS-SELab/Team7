...
if($company=="aapl")
{
	$sql = "SELECT * FROM logos WHERE company = 'Apple' LIMIT 1";
}


if($company=="googl")
{
	$sql = "SELECT * FROM logos WHERE company = 'Google' LIMIT 1";
}


if($company=="rfem")
{
	$sql = "SELECT * FROM logos WHERE company = 'RiverFront' LIMIT 1";
}
...
$json = str_replace('\\/', '/', json_encode($dbdata, JSON_PRETTY_PRINT));

$json = wordwrap($json, 150 );
